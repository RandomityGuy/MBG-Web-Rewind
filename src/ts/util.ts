...
export abstract class Scheduler {
	...
	schedule(time: number, callback: () => any, stringId: string = null) {
		let id = Math.random();
		this.scheduled.push({ id, stringId, time, callback });
		return id;
	}
	...
	clearScheduleId(id: string) {
		for (let i = 0; i < this.scheduled.length; i++) {
			if (this.scheduled[i].stringId === id) this.scheduled.splice(i--, 1);
		}
	}
}